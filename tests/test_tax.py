...
def test_first_buy_sets_initial_weighted_average():
    # arrange
    ...
    # assert
    assert result.tax == Money("0.00"), "Tax on purchase must be zero."
    ...
    assert result.new_state.accumulated_loss == Money("0.00"), (
...
def test_multiple_buys_calculate_weighted_average():
    ...
    assert result.tax == Money("0.00")


def test_sell_with_simple_profit_above_exemption_limit():
    # arrange
    initial_state = InvestmentState(
...
def test_sell_with_exemption_due_to_low_volume():
    # arrange
...
def test_sell_with_loss_generates_accumulated_loss():
    # arrange
    initial_state = InvestmentState(
        quantity=100,
        weighted_average_price=Money("50.00"),
        accumulated_loss=Money("0.00"),
    )
...
def test_sell_with_profit_fully_compensated_by_accumulated_loss():
    # arrange
...
def test_sell_with_profit_partially_compensated_by_accumulated_loss():
    # arrange
...
def test_sell_with_profit_exempt_but_compensating_loss():
    # arrange
...
def test_sell_above_exemption_limit_but_net_profit_is_zero():
    # arrange
...
def test_full_sequence_of_operations_and_state_management():
    current_state = InvestmentState()
    total_tax_paid = Money("0.00")
    ...
    assert result4.tax == Money("0.00"), "Tax must be zero due to low volume exemption."
    assert result4.new_state.accumulated_loss == Money("0.00"), (
    ...
    assert total_tax_paid == Money("0.00")


def test_process_operations_batch_extracts_taxes():
    # arrange
    operations_batch = [
